...
export default function Phonetics({ phonetics }: { phonetics: PhoneticRes }) {
	const phoneticsWithSound =
		phonetics && phonetics.filter((phonetic) => phonetic.audio?.length !== 0 && phonetic.text?.length !== 0);
	const audioUrl = phoneticsWithSound && (phoneticsWithSound[0]?.audio ? phoneticsWithSound[0]?.audio : "");
